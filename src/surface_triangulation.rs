...
pub fn edge_from_target<T:Clone>(target: EdgeTarget, triangulation: &SurfaceTriangulation<T>) -> Edge<Segment<T>> {
...
pub struct SurfaceTriangulation<T> {
    ...
    pub qeds: Qeds<Segment<T>, ()>,
    pub boundary_edge: EdgeTarget,
    pub bounds: Option<(Point, Point)>,
}

impl<T:Default+Clone> SurfaceTriangulation<T> {
    fn add_to_quad_unchecked(&mut self, mut edge_target: EdgeTarget, point: Point, data: T) -> EdgeTarget {
        unsafe {
    ...
    fn connect(&mut self, a: EdgeTarget, b: EdgeTarget) -> EdgeRefA<Segment<T>, ()> {
    ...
    fn add_point_to_edge(&mut self, edge_target: EdgeTarget, point: Point,data:T) -> EdgeTarget {
        unsafe {
            let point_a = self.qeds.edge_a_ref(edge_target).edge().point.point;
            let point_b = self.qeds.edge_a_ref(edge_target).sym().edge().point.point;
...
impl<T:Clone> SurfaceTriangulation<T> {
    ...
    pub fn new(a: (Point,T), b: (Point,T), c: (Point,T)) -> Self {
        ...
        let edge_a = qeds
            .make_edge_with_a(Segment::new(a.0,a.1.clone()), Segment::new(b.0,b.1.clone()))
            .sym()
            .target();
        let edge_b = qeds
            .make_edge_with_a(Segment::new(b.0,b.1), Segment::new(c.0,c.1.clone()))
            .target();
        let edge_c = qeds
...
impl<T> SurfaceTriangulation<T> {
    ...
    pub fn qeds(&self) -> Option<&Qeds<Segment<T>, ()>> {
        Some(&self.qeds)
    }

    pub fn some_edge_a(&self) -> Option<EdgeRefA<Segment<T>, ()>> {
    ...
    pub fn locate(&self, point: Point) -> Option<EdgeRefA<Segment<T>, ()>> {
        use rand::Rng;
    ...
    pub fn locate_tri(&self, point: Point) -> Option<EdgeRefA<Segment<T>, ()>> {
    ...
    pub fn boundary(&self) -> BoundaryIter<Segment<T>, ()> {
...
impl<'a,T> Iterator for TriangleIter<'a,T> {
    type Item = EdgeRefA<'a, Segment<T>, ()>;
    fn next(&mut self) -> Option<Self::Item> {
        loop {
...
impl<'a, T,BData> EdgeRefA<'a, Segment<T>, BData> {
...
fn show_triangle<T>(triangle: EdgeRefA<Segment<T>, ()>) -> String {
    format!(
        "{}-{}-{}",
...
fn show_edge<T>(edge: EdgeRefA<Segment<T>, ()>) -> String {
    format!(
        "{}-{}",
        edge.edge().point.point(),
        edge.sym().edge().point.point(),
